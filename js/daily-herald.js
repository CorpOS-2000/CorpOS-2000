...
function renderFrontPage() {
  const feed = ccrGetNewsFeed(20);
  const active = ccrListContracts((c) => c.status === 'active');
  const completed = ccrListContracts((c) => c.status === 'completed').slice(-5).reverse();
  ...
  <div class="dh-breaking">
    ...
    <span class="dh-breaking-text">${breaking ? escapeHtml(breaking.headline) : 'Welcome to The Daily Herald. No recent news.'}</span>
  </div>

  <table class="dh-layout" cellpadding="0" cellspacing="0">
    <tr>
      <!-- MAIN COLUMN -->
      <td class="dh-main">
        ...
        ${feed.length ? feed.map((item) => {
          ...
          const icon = item.kind === 'contract_created' ? '📋' : item.kind === 'contract_completed' ? '✅' : item.kind === 'negotiation' ? '🤝' : item.kind === 'contract_cancelled' ? '❌' : '📰';
        ...
        }).join('') : '<div class="dh-empty">No headlines yet. Contract activity will appear here.</div>'}
      </td>

      <!-- SIDEBAR -->
      <td class="dh-sidebar">
        <div class="dh-widget">
          <div class="dh-widget-title">Active Contracts</div>
          ${active.length ? active.map((c) => {
            const issuer = window.AXIS?.resolveContact?.(c.issuerActorId)?.name || c.issuerActorId;
            return `<div class="dh-widget-row">${escapeHtml(issuer)} — ${escapeHtml(formatMoney(ccrContractTotal(c)))}</div>`;
          }).join('') : '<div class="dh-widget-empty">None</div>'}
        </div>
        <div class="dh-widget">
          <div class="dh-widget-title">Recent Deals</div>
        ...
        <div class="dh-widget">
          <div class="dh-widget-title">System Alerts</div>
          ${feed.filter((f) => f.kind === 'contract_cancelled' || f.kind === 'negotiation').length
            ? feed.filter((f) => f.kind === 'contract_cancelled' || f.kind === 'negotiation').slice(0, 5).map((f) =>
              `<div class="dh-widget-row dh-alert">${escapeHtml(f.headline)}</div>`
            ).join('')
            : '<div class="dh-widget-empty">No alerts</div>'}
        </div>
      </td>
    </tr>
  </table>`;
